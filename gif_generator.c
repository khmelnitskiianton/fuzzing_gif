...
static const uint16_t MAX_SIZE_IMAGE = 50;               // max height/width of image
static const uint16_t IMAGE_SHIFT = 3;                   // shift of image in Image Descriptor
...
static const uint8_t TERMINATED_BYTE = 0x00;             // 0x00 - end byte for every block in gif
// Constants for probability in random generation P = 1/prob
static const int BASE_PROBABILITY = 2;
static const int AVERAGE_PROBABILITY = 4;
static const int SMALL_PROBABILITY = 6;
...
void generate_gif(gif_object_t *gif)
{
    ...
    if (!(rand() % AVERAGE_PROBABILITY))
        amount_of_image_descriptors = 0; // set random: one or more images
...
static void generate_gif_logical_screen_descriptor(gif_object_t *gif)
{
    ...
    gif->width = width;
    gif->height = height;
    // writes 2byte number to buffer with Little Endian
    memcpy((gif->data) + (gif->pos), (uint8_t *)&(width), sizeof(uint16_t));
    gif->pos += 2;
    ...
    uint8_t packed_field = (uint8_t)((((((global_color_table_flag << 3) + color_resolution) << 1) + sort_flag) << 3) +
                                     size_of_global_color_table);
    (gif->data)[(gif->pos)++] = packed_field;

    // Set Background Color Index & PixelAspect Ratio
    uint8_t background_color_index =
        (uint8_t)(rand() % (1 << (size_of_global_color_table + 1))); // index of background color from global table
...
void generate_gif_image_descriptor(gif_object_t *gif, uint32_t uncompressed_size_block)
{
    ...
    if (!(rand() % BASE_PROBABILITY)) // Turn on/off shift
    {
        ...
        if (!(rand() % BASE_PROBABILITY))
            shift_pos = BIG_IMAGE_SHIFT; // turn on/off BIG SHIFT
        left = shift_pos;
        top = shift_pos;
    }
    memcpy((gif->data) + (gif->pos), (uint8_t *)&(left),
           sizeof(uint16_t)); // writes 2byte number to buffer with Little Endian
    gif->pos += 2;
    memcpy((gif->data) + (gif->pos), (uint8_t *)&(top), sizeof(uint16_t));
    gif->pos += 2;

    // Write width & height dont generate
    uint16_t new_width = gif->width;
    uint16_t new_height = gif->height;
    if (rand() % AVERAGE_PROBABILITY) // make random width & height
    {
        new_height = (uint16_t)(rand() % UINT8_MAX);
        new_width = (uint16_t)(rand() % UINT8_MAX);
    }
    // writes 2byte number to buffer with Little Endian
    memcpy((gif->data) + (gif->pos), ((uint8_t *)&(new_width)), sizeof(uint16_t));
    gif->pos += 2;
    memcpy((gif->data) + (gif->pos), ((uint8_t *)&(new_height)), sizeof(uint16_t));
    gif->pos += 2;
    ...
    uint8_t packed_field =
        (uint8_t)((((((((local_color_table_flag << 1) | interlace_flag) << 1) | sort_flag) << 2) | reserve) << 3) |
    ...
    uint8_t lzw_minimum_code_size =
        ((gif->data)[PACKAGE_FIELD_POS] & MASK_COLOR_TABLE_SIZE) + 1; // because formula 2^(n+1)
    ...
    (gif->data)[(gif->pos)++] = TERMINATED_BYTE; // set 00 byte for end of image data
}

static void generate_gif_sub_block(gif_object_t *gif, uint32_t uncompressed_size_block)
{
    if (!uncompressed_size_block)
        return;
    ...
    (gif->pos)++;
    gif_lzw_compress(uncompressed_buffer, uncompressed_size_block, (gif->data) + (gif->pos), &compressed_size_block,
                     gif->lzw_minimum_code_size); // writes to buffer compressed image data.
    gif->pos = gif->pos + compressed_size_block;  // set to new pos
    (gif->data)[pos_for_size_of_block] =
        (uint8_t)compressed_size_block; // write size of image data block before data block
}
...
void generate_gif_graphics_control_extension(gif_object_t *gif)
{
    ...
    const uint8_t extension[] = {0x21, 0xF9};
    memcpy(gif->data, extension, sizeof(extension));
    gif->pos += sizeof(extension);
    size_t byte_size_pattern = 5;
    for (size_t i = 0; i < byte_size_pattern; i++)
    {
        gif->data[(gif->pos)++] = (uint8_t)(rand() % UINT8_MAX);
    }
    gif->data[(gif->pos)++] = TERMINATED_BYTE;
}

void generate_gif_plain_text_extension(gif_object_t *gif)
{
    //{0x21, 0x01, 0x0C, 0x00, 0x00, 0x00,
    // 0x00, 0x64, 0x00, 0x64, 0x00, 0x14,
    // 0x14, 0x01, 0x00, 0x0B, 0x68, 0x65,
    // 0x6C, 0x6C, 0x6F, 0x20, 0x77, 0x6F,
    // 0x72, 0x6C, 0x64, 0x00}; pattern

    const uint8_t extension[] = {0x21, 0x01}; // signature of this extension
    memcpy(gif->data, extension, sizeof(extension));
    gif->pos += sizeof(extension);
    size_t byte_size_pattern = 25;
    for (size_t i = 0; i < byte_size_pattern; i++)
    {
        gif->data[(gif->pos)++] = (uint8_t)(rand() % UINT8_MAX);
    }
...
void generate_gif_application_extension(gif_object_t *gif)
{
    ...
    // 0x45, 0x32, 0x2E, 0x30, 0x03, 0x01, 0x05, 0x00, 0x00};
    const uint8_t extension[] = {0x21, 0xFF};
    memcpy(gif->data, extension, sizeof(extension));
    gif->pos += sizeof(extension);
    size_t byte_size_pattern = 16;
    for (size_t i = 0; i < byte_size_pattern; i++)
    {
        gif->data[(gif->pos)++] = (uint8_t)(rand() % UINT8_MAX);
    }
...
void generate_gif_comment_extension(gif_object_t *gif)
{
    ...
    const uint8_t extension[] = {0x21, 0xFE};
    memcpy(gif->data, extension, sizeof(extension));
    gif->pos += sizeof(extension);
    size_t byte_size_pattern = 10;
    for (size_t i = 0; i < byte_size_pattern; i++)
    {
        gif->data[(gif->pos)++] = (uint8_t)(rand() % UINT8_MAX);
    }
