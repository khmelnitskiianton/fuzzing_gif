...
Прочитал readme к [AFL](https://github.com/google/AFL).
Установка в соответствии с [инструкцией](https://github.com/google/AFL/blob/master/docs/INSTALL). `afl-fuzz` - установлен.

**Подготовка:**
+ Написал программу на C++ с чтением числа из stdin и условным переходом содержащим `abort`. 
+ Добавил `Makefile`.
+ В соответствии с [инструкцией по запуску](https://github.com/google/AFL/blob/master/docs/QuickStartGuide.txt) был изменен компилятор с gcc на afl-gcc в `Makefile` и фаззинг с помощью программы `afl-fuzz`. Также добавлены директории с тестовыми примерами.

Все компилируется, начало попыток фаззинга
...
**Первый успешный фаззинг простой тестовой программы:**
![[images/20240628201228.png]]
Прочитал инструкцию об [информации на экране](https://github.com/google/AFL/blob/master/docs/status_screen.txt) .

### LibFuzzer и AFL++

Начинаю изучение LibFuzzer/AFL++
LibFuzzer:
...
После прохождения гайда по LibFuzzer, понял что его юзать не надо будет хоть там и есть кастомные мутаторы.
Устанавливаю AFL++ и читаю гайд:
+ Успешно установлен AFLplusplus и запущена тестовая программа из 1 части.
+ Прочитаны основы и решено начать тестировать выбранную программу постепенно увеличивая инструментарий.
## Постановка задачи
*****
**Задача:**
...
В фаззере AFL++ есть такая возможность. Нужно написать мутацию, которая учитывая формат входных данных, принимаемый анализируемой программы, 
поддерживает 2 режима работы и применяет их (может быть один первый режим если он позволяет достичь прироста путей):
...
```bash
cmake ../ -DWITH_OPENSLIDE_SUPPORT=On #было
#===========================================
#export AFL_USE_ASAN=1
cmake ../ -DWITH_OPENSLIDE_SUPPORT=On #стало
make
sudo make install
```
![[images/20240708013018.png]]
3. В отдельной папке организую фаззинг.
Собрал корпус входных `*.gif` файлов и подключил соответственный стандартный словарь от aflpp. Запускаю aflpp(прохожу исправление всех ошибок из 1 части).
```bash
...
Пришлось в установленном afl-cov пайтон скрипте искать проблему - оказалось при обработке оно считывало все файлы из default/queue/ с маской `id:000000` НО afl генерит файлы `id_000000` из-за этого он не мог считать ниодного файла. Поправил в самом коде, далее пришлось смотреть какие команды он вызывает и как объединяет файлы для lcov. Делал он это запуская 
...
В этот момент стало понятно, что лучше взять другой проект для фаззинг :cry: :cry: :cry: :cry:

Чему я научился - разные инструменты покрытия, а также компиляция с afl++ в cmake!

## Фаззинг проекта [chafa](https://github.com/hpjansson/chafa), Success
...
export AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES=1
export AFL_SKIP_CPUFREQ=1
afl-fuzz -i input -o output_1 -- /usr/local/bin/chafa  @@
```
...
**Запуск с подключением стандартного словаря:**
```bash
export AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES=1
export AFL_SKIP_CPUFREQ=1
