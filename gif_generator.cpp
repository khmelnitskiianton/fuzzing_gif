...
static const uint8_t  MAX_SUB_BLOCK_SIZE = 0xFF;    //Block size [0,255]
static const uint16_t SIZE_OF_UNC_DATA   = 0x300;   //Size of uncompressed data - fixed, 0x300 after compression <= 250 - ideal for image with one block
...
void generate_gif(gif_object_t *gif)
{
    ...
    if (rand()%10 == 0)  //1/8 probability because this block crashes file!
    {
...
static void generate_gif_title(gif_object_t *gif)
{
    memcpy(gif->data, "GIF89a", 6);//I choose only recent signature-<GIF89a> 
    gif->pos += 6; 
}
...
static void generate_gif_logical_screen_descriptor(gif_object_t *gif)
{
    ...
    uint16_t height = 1+(uint16_t) (rand() % MAX_SIZE_IMAGE);
    gif->width  = width;
    gif->height = height;
...
static void generate_gif_sub_block(gif_object_t *gif, uint32_t uncompressed_size_block)
{
    ...
    (gif->data)[pos_for_size_of_block] = compressed_size_block; //write size of image data block before data block
}

//Generates sequence for settings of Program Block and Animation Block
//I take them like pattern from https://www.matthewflickinger.com/lab/whatsinagif/bits_and_bytes.asp
static void generate_gif_graphics_control_extension(gif_object_t *gif)
{
    const uint8_t gce_program[] = {0x21, 0xFF, 0x0B, 0x4E, 0x45, 0x54, 0x53, 0x43, 0x41, 0x50, 0x45, 0x32, 0x2E, 0x30, 0x03, 0x01, 0x05, 0x00, 0x00};
    memcpy(gif->data, gce_program,  sizeof(gce_program));
    gif->pos += sizeof(gce_program); 

    const uint8_t gce_animation[] = {0x21, 0xF9, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00};
    memcpy(gif->data, gce_animation, sizeof(gce_animation));
    gif->pos += sizeof(gce_animation); 
}